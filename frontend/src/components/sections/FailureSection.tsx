        ...
        <div className="flex flex-wrap items-end justify-between gap-8 border-t border-risk/40 pt-8">
          <div>
            ...
            <SplitText
              ...
              type="chars"
              stagger={0.025}
              className="mt-5 font-display text-display-md font-medium uppercase leading-[0.88] text-chalk"
            >
              Failure is data.
            </SplitText>
          </div>
          <p className="max-w-[40ch] text-pretty text-base leading-relaxed text-chalk/50">
          ...
          {FAILURE_RECORDS.map((f) => (
            <li key={f.id} data-failure className="panel p-8">
              <div className="flex items-baseline justify-between gap-4">
                ...
                <span className="font-mono text-meta uppercase text-chalk/50">{f.id}</span>
              </div>
              <p className="mt-2 font-mono text-meta uppercase text-chalk/50">{f.department}</p>

              <dl className="mt-8 space-y-5">
                <div>
                  <dt className="font-mono text-meta uppercase text-risk">Result</dt>
                  <dd className="mt-1.5 text-base text-chalk/75">{f.result}</dd>
                </div>
                <div>
                  <dt className="font-mono text-meta uppercase text-chalk/50">Cause</dt>
                  <dd className="mt-1.5 text-base text-chalk/75">{f.cause}</dd>
                </div>
                <div>
                  <dt className="font-mono text-meta uppercase text-signal">Lesson</dt>
                  <dd className="mt-1.5 text-base text-chalk">{f.lesson}</dd>
                </div>
              </dl>
              ...
              <p className="border-t border-chalk/15 pt-4 text-sm leading-relaxed text-validated">
                {f.ruleAdded}
              </p>
            </li>
          ))}
